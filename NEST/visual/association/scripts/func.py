...
def find_center(Layer):
    X_center = Layer[area][X_area] / sum(Layer[step]) / 2
    print X_center
    for i in range(int(Layer[area][Y_area] / 4)):
        X_center += (Layer[area][X_area] / sum(Layer[step]))
    return X_center


def detector_radius(Layer, radius):
    column_list = list()
    center = find_center(Layer)
    layer_step = Layer[area][X_area] / sum(Layer[step])
    lower_left = center - radius - radius * layer_step
    for i in range(lower_left, 2*radius + 1 + lower_left, 1):
        for j in range(2*radius + 1):
            column_list.append(i + layer_step * j)
    print "center", center, "list", sorted(column_list)
...
def build_model():
    ...
    # Build another parts
    for part in (V4[Glu], testArea[DA], StartV[Glu]):
    ...
    for layer in Cortex:
        ...
        for Y_border in range(0, layer[area][Y_area], 2):
            for X_border in range(0, layer[area][X_area], sum(layer[step])):
                GABA_neurons_list_ID = list()
                Glu_neurons_list_ID = list()
                for X in range(X_border, X_border + layer[step][Glu_step], 1):
                    Glu_neurons_list_ID.append(dictPosition_NeuronID[ (float(X), float(Y_border),   float(layerNumberZ)) ])
                    Glu_neurons_list_ID.append(dictPosition_NeuronID[ (float(X), float(Y_border+1), float(layerNumberZ)) ])
                if layer[step][GABA]:
                    for X in range(X_border + layer[step][Glu_step], X_border + sum(layer[step]), 1):
                        GABA_neurons_list_ID.append(dictPosition_NeuronID[ (float(X), float(Y_border),   float(layerNumberZ)) ])
                        GABA_neurons_list_ID.append(dictPosition_NeuronID[ (float(X), float(Y_border+1), float(layerNumberZ)) ])
                    layer[GABA][column] = GABA_neurons_list_ID
                layer[Glu][column] = Glu_neurons_list_ID
                column+=1
        layerNumberZ-=1
        realLayer += 1
...
def connect(pre, post, syn_type=GABA, weight_coef=1):
    ...
    nest.Connect(pre, post,
                 conn_spec=conn_dict,
                 syn_spec=types[syn_type][3] if syn_type == DA_ex else types[syn_type][0])
...
def connect_multimeter(part, name):
    multimeters[name] = nest.Create('multimeter', params=multimeter_param)  # ToDo add count of multimeters
    nest.Connect(multimeters[name], (part[:2]))
    logger.debug("Multimeter => {0}. On {1}".format(name, part[:2]))
