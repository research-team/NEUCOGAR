...
def connect(pre, post, syn_type=GABA, weight_coef=1):
    ...
    conn_dict = {'rule': 'all_to_all',
                 'multapses': True}
    nest.Connect(pre, post, conn_spec=conn_dict, syn_spec=types[syn_type][0])
...
def simulate():
    ...
    for t in np.arange(0, T, dt):
        ...
        for data in sorted(nest.GetStatus(nest.GetConnections((Cortex[L5][Glu][10][0], ) ))):
            print "from {0} to {1} weight {2} | delay {3} | tau_plus {4}".format(data['source'], data['target'], data['weight'], data['delay'], data['tau_plus'])
#
        end = clock()
