...
# Set connectomes
for index in motor_cortex.getColumnsIndexes():
	motor_cortex.columns(index).layers(L6).nuclei(Glu).connect(striatum.nuclei(GABA_D1), synapse=Glutamatergic, weight=0.1)
	motor_cortex.columns(index).layers(L6).nuclei(Glu).connect(striatum.nuclei(GABA_D2), synapse=Glutamatergic, weight=0.1)
	motor_cortex.columns(index).layers(L6).nuclei(Glu).connect(striatum.nuclei(GABA_D2), synapse=Glutamatergic, weight=0.08)
	motor_cortex.columns(index).layers(L6).nuclei(Glu).connect(stn.nuclei(Glu), synapse=Glutamatergic, weight=1)
	motor_cortex.columns(index).layers(L6).nuclei(Glu).connect(thalamus.nuclei(Glu_0), synapse=Glutamatergic, weight=1, rec_weight=True)

	thalamus.nuclei(Glu_0).connect(motor_cortex.columns(index).layers(L4).nuclei(Glu), synapse=Glutamatergic, weight=1)
...
gpi.nuclei(GABA).connect(thalamus.nuclei(Glu_0), synapse=GABAergic, weight=-3)

snr.nuclei(GABA).connect(thalamus.nuclei(Glu_0), synapse=GABAergic, weight=-3)
...
for index in motor_cortex.getColumnsIndexes():
	motor_cortex.columns(index).layers(L4).nuclei(Glu).ConnectPoissonGenerator(rate=300, conn_percent=50, weight=100)
...
for index in motor_cortex.getColumnsIndexes():
	motor_cortex.columns(index).layers(L4).nuclei(Glu).ConnectDetector()
...
# Connect multimeters
for index in motor_cortex.getColumnsIndexes():
	motor_cortex.columns(index).layers(L6).nuclei(Glu).ConnectDetector()
