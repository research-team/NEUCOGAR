...
def generate_neurons(NNumber):
    ...
    parts_no_dopa = PGI + BNST + Amy + Thalamus + PVN + LTD + PrH + AcbCore + Prefrontalcortex + AcbShell +\
                    ...
                    gpe + gpi + stn + amygdala + (vta[vta_GABA0], vta[vta_GABA1], vta[vta_GABA2], snc[snc_GABA]) + \
    ...
    parts_with_dopa = (LC[LC_NA_0], LC[LC_D1], LC[LC_D2], LC[LC_NA_1], RN[RN_5HT], Ab[Ab_NA], VTA[VTA_DA_0],\
                       VTA[VTA_DA_1], Aa[Aa_NA], vta[vta_DA0], vta[vta_DA1], snc[snc_DA])
    ...
    all_parts = tuple(sorted(parts_no_dopa + parts_with_dopa + parts_no_5HT + parts_with_5HT))
    ...
    # Parts with dopamine
    for part in parts_with_dopa + parts_with_5HT + parts_no_5HT:
