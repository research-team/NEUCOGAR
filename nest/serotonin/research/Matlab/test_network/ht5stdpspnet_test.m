% DA- and ST(HT5)-modulated spiking network of 1000 quadratic leaky IAF neurons
% based on [dasp.net by Izhikevich]
...
s=[ones(Ne,M);-ones(Ni,M)];         % initial synaptic weights
...
STDP = zeros(N,1001+D);
v = -65*ones(N,1);                      % initial values of membrane potential (-65 mV ground)
u = 0.2.*v;                             % initial values of recovery variable
firings=[-D 0];                         % spike timings

T=100;           % the duration of experiment
DA=0;           % initial level of dopamine
ST=0;           % initial level of serotonine (5-HT)
