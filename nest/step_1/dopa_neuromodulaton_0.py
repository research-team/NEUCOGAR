...
if pg_flag:
    pg_fast = nest.Create("poisson_generator", 1, {"rate": 270.})  # ToDo find out nu_ex
    pg_slow = nest.Create("poisson_generator", 1, {"rate": 89., })  # 'start': 1., 'stop': 200.})
    ...
    nest.Connect(nest.Create("poisson_generator", 1, {"rate": 19.}) , cortex, syn_spec=gen_static_syn)
    ...
    # neuromodulation
    if vt_flag:
        nest.Connect(nest.Create("poisson_generator", 1, {"rate": 1000.,'start': 500., 'stop': 600.}), snc, syn_spec=gen_static_syn)
    # else:
    ...
    f_gen_connect(thalamus, rang=np.arange(3, T, 20.))
